...
nb::tuple trtllmGenGenerationPreprocessBinding(torch::Tensor qkv_input, torch::Tensor workspace,
    ...
    int64_t total_num_blocks, int64_t kv_factor, bool need_build_kv_cache_metadata, bool is_ragged_gen)
{
    auto result = [&]()
    {
        nb::gil_scoped_release release;
        return torch_ext::trtllmGenGenerationPreprocess(qkv_input, workspace, sequence_lengths,
            ...
            need_build_kv_cache_metadata, is_ragged_gen);
    }();
...
void initBindings(nb::module_& m)
{
    ...
    m.def("attention", &torch_ext::attention,
        ...
        nb::arg("spec_decoding_target_max_draft_tokens") = std::nullopt, nb::arg("is_ragged_gen") = false,
        nb::arg("max_gen_q_len") = 0, nb::arg("gen_cu_q_seqlens") = std::nullopt, "Multi-head attention operation",
    ...
    m.def("trtllm_gen_generation_preprocess", &trtllmGenGenerationPreprocessBinding, nb::arg("qkv_input"),
        ...
        nb::arg("is_ragged_gen") = false, "Fused nanobind generation preprocess for trtllm-gen attention.");
}
} // namespace tensorrt_llm::nanobind::thop
