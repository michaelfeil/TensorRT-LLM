...
void attention(torch::Tensor q, std::optional<torch::Tensor> k, std::optional<torch::Tensor> v, torch::Tensor& output,
    ...
    // Ragged per-request generation q-lens (speculative decoding). Trailing with
    // defaults so existing positional/keyword callers are unaffected.
    bool const is_ragged_gen = false, int64_t const max_gen_q_len = 0,
    std::optional<torch::Tensor> gen_cu_q_seqlens = std::nullopt);
