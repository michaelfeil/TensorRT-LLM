...
class KVCacheV2Scheduler(RequestScheduler):
    ...
    def _align_chunk_to_mm_block(
        ...
        if cumsum is None:
            block = find_multimodal_span_containing_boundary(
                getattr(req, "multimodal_positions", None) or [],
                getattr(req, "multimodal_lengths", None) or [],
                end_abs,
            )
            if block is None:
                return chunk_size
            block_start_abs, block_end_abs = block
        else:
            ...
            while block_end_abs < prompt_len:
                if (int(cumsum[block_end_abs].item()) -
                        int(cumsum[block_end_abs - 1].item())) != 1:
                    break
                block_end_abs += 1
            ...
            while block_start_abs > 1:
                if (int(cumsum[block_start_abs - 1].item()) -
                        int(cumsum[block_start_abs - 2].item())) != 1:
                    break
                block_start_abs -= 1
